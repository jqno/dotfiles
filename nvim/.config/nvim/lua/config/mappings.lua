...
local function define_mappings()
    ...
    map(modes.n, '[d',
        centered(function() vim.diagnostic.jump({ count = -1, float = true, severity = { min = vim.diagnostic.severity.WARN } }) end),
    ...
    map(modes.n, '<leader>fM', function() vim.cmd.FzfLua('marks') end, { desc = 'find Vim marks' })
    ...
    map(modes.n, '<leader>fu', function() require('undotree').open({ command = 'leftabove 30vnew' }) end,
        { desc = 'open undo tree' })
